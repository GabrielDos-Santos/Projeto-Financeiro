...
**Roadmap completo: MVP (Fases 0–14) + expansão (17, 15 e 16).** Última fase concluída: **Fase 16 — Família/households** (19/07/26; lint + typecheck + build verdes, 28 rotas). Casa com admin + membros, RLS de SELECT estendida (admin lê tudo; membro lê agregados + contas compartilhadas), convites por link com token hasheado, dashboard da família com funções agregadas `SECURITY DEFINER`, rotas `/familia`, `/familia/dashboard` e `/convite/[token]` (decisões 83–90).
**Próxima fase:** nenhuma no roadmap combinado — evoluções mapeadas nos docs (OFX, Web Push, `household_shared_cards`, e-mail transacional de convite, quebra por membro etc.) entram sob demanda.
...
1. **Aplicar as migrations `0010`–`0015`**: `npx supabase db push`. `0010`/`0011` **não** mudam colunas — não precisam de `npm run db:types`. **`0012` (Fase 17) muda colunas e cria tabela** (`affects_balance`, `import_batches`, `import_batch_id`, `import_hash`), **`0013`** adiciona `credit_cards.invoice_name_by_due_month` (decisão 68), **`0014` (Fase 16) cria 4 tabelas + 2 enums + 12 funções e estende as policies de SELECT** (decisões 83–90) e **`0015`** recria `v_entries` expondo `created_at` (decisão 94) — **rodar `npm run db:types` depois do push** e conferir que bate com o `src/types/database.ts` hand-patched nestas sessões. `0010` evita dupla contagem do pagamento de fatura nos relatórios (decisão 34). `0011` concede `execute` de `generate_recurring_transactions()` ao `service_role` — sem ela o botão "Gerar agora" falha com _permission denied_ (ver decisão 38).
...
6. **Fase 16 (Família) — critério de pronto com 3 usuários reais** (após aplicar a `0014`): 1 admin + 2 membros; admin vê linha a linha dos 3 em `/transacoes`; membro vê agregados em `/familia/dashboard` + somente contas compartilhadas; membro NÃO acessa linha não compartilhada; ninguém edita dado alheio (tentar update/delete → falha); convite expira/reenvio invalida o anterior; membro sai sozinho pela UI.
...
| `supabase/migrations/0014_households.sql`                                                                                          | Fase 16 — households: 2 enums, 4 tabelas, funções de parentesco/fluxo/agregado (`SECURITY DEFINER`), RLS de SELECT estendida em 9 tabelas (decisões 83–90). **Pende `db push` + `db:types`** (tabelas/enums/funções novas)              |
| `supabase/migrations/0015_v_entries_created_at.sql`                                                                                | Recria `v_entries` acrescentando `created_at` (ordenar por "adicionadas por último" — decisão 94). **Pende `db push` + `db:types`** (coluna nova na view)                                                                              |
...
81. _(correção 19/07/26, Fase 6/17)_ **Compra no cartão (à vista ou parcelada) cuja fatura já estava paga nascia `pending` para sempre.** `createCardPurchase`/`createCardInstallmentPurchase` sempre inseriam `status: "pending"`, sem checar se a fatura de destino (via `get_or_create_invoice`) já tinha `status = 'paid'` — cenário real: usuário lança hoje uma compra parcelada com data de compra no passado (ex.: 10x de 10/abril), cujas primeiras parcelas caem em faturas de meses já pagos. Como item de cartão é travado (decisão 36 — status só muda pelo pagamento da fatura), essas parcelas ficavam pendentes sem nenhuma ação manual possível na UI. O hub de backfill (decisão 62) não resolve: ele **recusa** faturas já pagas ("reabra-a antes de reconstruir"), pensado pro caso oposto (reconstruir ANTES de pagar). Corrigido detectando por fatura: se `credit_card_invoices.status === 'paid'` no momento da criação, a parcela (ou a compra à vista) nasce `paid` + `affects_balance = false` direto (mesmo padrão histórico do backfill), em vez de esperar um `paidCount` informado à mão. Compras cuja fatura ainda está aberta continuam nascendo `pending` normalmente — nada muda no caminho comum.
82. _(correção 19/07/26, Fase 15)_ **Teste em dispositivo real (prints do usuário) achou vazamento horizontal em várias telas** — o checklist da Fase 15 previa isso (item "testar no deploy real… não só no DevTools", decisão 80), e foi exatamente o caso: sem `min-width: 0`, um item de `grid`/`flex` nunca encolhe abaixo do próprio `min-content` — um SVG de gráfico, um texto sem quebra ou um `<Select>` de largura fixa lado a lado com outro elemento alargam a COLUNA inteira (e a página junto) em telas estreitas, obrigando a diminuir o zoom pra ver tudo. Correções:
    - **Guard global em `globals.css`**: `.grid > * { min-width: 0 }` — cobre dashboard (últimas movimentações, cartões, gráficos), `/contas` e qualquer grid futuro sem precisar anotar `min-w-0` item por item; `<main>` (`(app)/layout.tsx`) ganhou `overflow-x-hidden` como cinto-e-suspensório.
    - **`/transacoes`**: barra de filtros + botão "Novo lançamento" virou coluna no mobile (o botão ficava sobreposto à busca); barra de seleção em massa ganhou `flex-wrap`; barra de paginação ("Por página"/contagem/navegação) virou coluna centralizada no mobile em vez de uma linha só que não cabia e desalinhava.
    - **`Dialog`** (`components/ui/dialog.tsx`) ganhou `overflow-x-hidden` — mitiga qualquer filho largo em qualquer formulário, incluindo o de editar conta.
    - **`ColorPicker`/`IconPicker`** (usados no form de conta, categoria, cartão): `grid-cols-9`/`grid-cols-8` fixos trocados por `flex-wrap`/`grid-cols-[repeat(auto-fill,…)]` — número de colunas fixo não cabia em ~375px e empurrava o diálogo pra fora da tela (era exatamente o "editar fora do padrão" reportado em Contas).
    - **`AccountCard`**: badge "Arquivada" ganhou `shrink-0` (nome longo + badge sem isso comprimia o badge até cortar o texto).
    - **`LimitBar`** (cartões): linha "Usado/Disponível" ganhou `flex-wrap`.
83. _(Fase 16)_ **Modelo de papéis e visibilidade** (era a "decisão 27" do `docs/ARQUITETURA-EXPANSAO.md` — as decisões 25–30 daquele doc colidiam com as 25–30 reais deste changelog e foram renumeradas para este bloco, conforme previsto na decisão 55; as antigas 25/26 sobre PWA já foram absorvidas pelas decisões 74–80 da Fase 15): **admin da casa lê tudo de todos os membros, linha a linha; membro lê o dashboard agregado da família + as contas marcadas como compartilhadas.** Ninguém escreve dado alheio; consentimento explícito no aceite do convite (a tela diz o que o admin passará a ver); quem gerencia a lista de contas compartilhadas é o admin. Compartilhamento v1 = **contas** (compras de cartão aparecem para membros só nos agregados; `household_shared_cards` é evolução).
84. _(Fase 16)_ **Checagens de parentesco em funções `SECURITY DEFINER`** (era a "decisão 28" do doc de expansão): policies que consultam outras tabelas com RLS recursam — `is_member_of`/`is_admin_of`/`is_admin_over`/`account_shared_with_me`/`account_owned_by_member_of`/`shares_household_with` são `stable security definer` com `set search_path = ''` e EXECUTE só para `authenticated`. **Agregados da família via funções DEFINER com guarda de membership na primeira linha** (`get_household_monthly_summary`/`get_household_category_breakdown`/`get_household_monthly_series`) — nunca view `security definer` (não teria como filtrar por membership). A semântica dos agregados espelha `v_monthly_summary`/0010: competência, sem transfer, sem canceladas, excluindo pagamento de fatura, mãe parcelada fora (cada parcela no seu mês). Categorias de usuários diferentes com o MESMO nome agregam juntas no breakdown ("Alimentação da casa").
85. _(Fase 16)_ **Escrita continua estritamente "own rows"** (era a "decisão 29"): a migration `0014` só troca policies de **SELECT** em `accounts`, `transactions`, `transaction_installments`, `categories`, `credit_cards`, `credit_card_invoices`, `budgets`, `goals` e `profiles` — cada uma vira `dono OU is_admin_over(user_id)` (+ `account_shared_with_me` em accounts/transactions; parcelas de conta compartilhada entram por `exists` na compra-mãe). Views `security_invoker` (decisão 23) herdam tudo automaticamente. **`profiles` foi além do doc**: membros da mesma casa se enxergam (`shares_household_with`) — sem isso a lista de membros não teria nome de ninguém.
86. _(Fase 16)_ **Convite por token com hash** (era a "decisão 30"): token de 32 bytes (base64url) só no link `/convite/<token>`; o banco guarda o sha-256 (`token_hash` unique). Expiração 7 dias; **reconvidar o mesmo e-mail deleta o convite pendente anterior** (o token antigo morre); rate limit perfil `:invite` (20/h) via `check_rate_limit`. **Desvio consciente do doc: v1 SEM e-mail transacional** — o doc assumia "Resend/provider já configurado", que não existe neste projeto; o admin gera o link e envia por onde quiser (WhatsApp/e-mail manual), exibido UMA vez na criação (não é reconstituível do hash). Envio automático é evolução.
87. _(Fase 16)_ **Criar casa e aceitar convite são funções `SECURITY DEFINER`** (`create_household`, `accept_household_invite`, + `get_invite_details` para a tela de consentimento) — uma policy de INSERT "user_id = uid" em `household_members` deixaria qualquer um se inserir em qualquer casa; sem policy de INSERT, só os fluxos validados inserem. **v1: uma casa por usuário**, garantido dentro das duas funções (`raise exception` se já há membership ativa) e na UI. O DB continua modelado para N casas (constraint é `unique (household_id, user_id)`), como no doc.
88. _(Fase 16)_ Guardas de saída/remoção: **o único admin não sai enquanto houver outros membros** (a casa ficaria sem gestão — remove os membros antes); **o único membro que sai encerra a casa inteira** (delete do household; cascade limpa membros/convites/compartilhamentos); admin remove membro mas nunca a si mesmo pela ação de remover (`neq user_id`). Papel de convite é sempre `member` na v1 (promover a admin/trocar papel = evolução; policy de UPDATE em `household_members` nem existe ainda).
89. _(Fase 16)_ **Efeito colateral documentado da RLS estendida**: as telas normais do admin (ex.: `/transacoes`, que lê `v_entries` sem filtro de usuário) passam a exibir também os lançamentos dos membros — é o "drill-down" previsto no doc, e é o que satisfaz o critério "admin vê linha a linha". Consequência: os NÚMEROS pessoais do admin (dashboard, relatórios) passam a somar os dados visíveis dos membros. `v_budget_usage` não mistura (o join é por `category_id`, e categorias são por usuário). Um filtro "só meus lançamentos" nas telas pessoais fica como evolução — no uso single-family real, o admin é quem quer ver tudo junto.
90. _(Fase 16)_ Rotas novas: `/familia` (criar casa · membros · convites pendentes com revogar · contas compartilhadas com gestão) e `/familia/dashboard` (cards de totais do mês + série de 6 meses + donut por categoria, reusando `IncomeExpenseBarChart`/`CategoryDonutChart`/`StatCard`), ambas RSC; `/convite/[token]` protegida pelo middleware (não logado cai no `/login?next=` e volta ao link — cobre "cadastra antes" do doc). Navegação: item "Família" na sidebar; no mobile a bottom bar virou **4 itens fixos + botão "Mais"** (sheet de baixo com Categorias/Recorrentes/Orçamentos/Metas/Relatórios/Família/Configurações) — antes, 5 rotas simplesmente não existiam na navegação mobile; agora todas as 11 são alcançáveis. Feature `households` segue o padrão feature-based (14ª pasta em `src/features/`).
91. _(correção 19/07/26, Fase 16)_ **RLS estendida sem identificação visual do dono era confuso na prática**: reportado pelo usuário logo após testar — `/transacoes` e `/contas` passaram a poder mostrar linhas de outro membro (admin, ou conta compartilhada) sem NENHUMA pista de quem era o dono, então "Ítau" (conta) e a compra "Mercado" pareciam simplesmente do próprio usuário. Corrigido com `getHouseholdMemberNames()` (`features/households/queries.ts` — mapa `user_id → nome` dos membros ativos, `null` fora de uma casa, o caso comum) e `ownerName()`/`MemberBadge` (`lib/households.ts`/`components/shared/member-badge.tsx`, compartilhados entre as duas features): uma pílula discreta com o nome aparece ao lado da conta/cartão SÓ quando a linha não é do próprio usuário — nenhuma mudança visual pra quem não está numa casa. De quebra, `AccountCard` escondeu editar/arquivar/excluir em contas de outro membro (a RLS de escrita já bloquearia — decisão 85 — e sem esconder a ação parecia disponível e falhava sem explicação).
92. _(melhoria 19/07/26)_ **Ordenação configurável em `/transacoes` + feed do dashboard corrigido** (pedido do usuário). (a) `fetchEntriesPage` ganhou um parâmetro `sort` (`ENTRY_SORT_OPTIONS`: mais recentes/mais antigas/maior valor/menor valor), com seletor próprio na barra de filtros (`ArrowDownUp`) — NÃO entra na contagem do badge "Filtros" nem no "Limpar", porque ordenação não é filtro. `date` é competência/vencimento (a view não expõe `created_at`), então "Mais antigas" + status pendente é justamente o que traz as parcelas mais atrasadas ao topo. Desempate estável por `id desc` mantém o offset sem pular/repetir linha; `sort` entrou na `queryKey` do TanStack e na condição de `initialData` (só reusa o SSR quando ordenação = padrão). (b) **`getRecentEntries` (dashboard "Últimas movimentações") passou a filtrar `date <= hoje`** — como `date` é a competência, parcelas AGENDADAS para meses/anos à frente (ex.: 12/12 vencendo em 2027) apareciam no topo de "As mais recentes", que deveria ser histórico recente, não previsão. O dashboard tem, assim, sua própria ordenação estável (recentes até hoje), independente do que o usuário ordenar em `/transacoes`.
93. _(correção 19/07/26, Fase 4)_ **Abas do drawer de novo lançamento cortadas no mobile**: as 4 abas (Despesa/Receita/Cartão/Transferência — "Transferência" é longa) estouravam a largura estreita do `Sheet` e ficavam cortadas dos dois lados (`justify-center` + `whitespace-nowrap`). A `TabsList` do drawer virou `grid` de 2 colunas no mobile (2×2, ou 2+1 sem cartão) e linha única a partir do `sm`, com `[&>button]:h-9` (alvo de toque). `SheetContent` ganhou `overflow-x-hidden` por segurança. A `TabsList` compartilhada (`components/ui/tabs.tsx`) não mudou — só o uso no drawer.
94. _(melhoria 19/07/26)_ **Ordenação "Adicionadas por último" (ordem de inserção) + agora o padrão** (pedido do usuário; supera a decisão 92). A view `v_entries` não expunha `created_at`, então ordenar por "mais recentes" era por `date` (competência) e jogava as parcelas agendadas para 2027 no topo. Migration `0015` recria `v_entries` acrescentando `created_at` (linha de transação → `transactions.created_at`; linha de parcela → `created_at` da **mãe**, já que `transaction_installments` não tem coluna própria). `ENTRY_SORT_OPTIONS` ganhou `created_desc` ("Adicionadas por último") como **novo padrão** — `date_desc` virou uma opção explícita ("Data (mais recente)"). Os dois feeds de dashboard também passaram a ordenar por `created_at` (o pessoal já filtrava `date <= hoje`). `create or replace view` só permite acrescentar coluna no fim — `created_at` entra após `affects_balance`.
95. _(Fase 16, melhoria 19/07/26)_ **Dashboards separados por escopo + filtro por membro** (pedido do usuário). (a) **Dashboard principal = pessoal**: `getRecentEntries` (feed "Últimas movimentações") passou a filtrar `user_id = auth.uid()` — a RLS estendida (decisão 89) fazia o admin ver lançamentos dos membros misturados no feed pessoal (ex.: um Pix da CRISTINA). (b) **Dashboard da família ganhou o feed "Últimas movimentações" com o nome do membro de cada linha** (`getHouseholdRecentEntries` + `HouseholdRecentEntries`), **só para o admin** (membro comum vê apenas agregados — decisão 83); lê `v_entries` direto (a RLS do admin já expõe as linhas), `date <= hoje`, ordenado por `created_at`. (c) **Filtro por membro em `/transacoes`, só para o admin**: `EntryFilters.userId` + select "Todos os membros" na barra (renderiza apenas quando `memberOptions` não é vazio — admin com >1 membro; para membro comum filtrar por outro daria lista vazia pela RLS). Refactor: `getHouseholdMemberNames` virou `getHouseholdContext` (`{ myUserId, isAdmin, members, memberNames }`), consumido por `/contas` e `/transacoes` — uma fonte só para badge de dono e opções de filtro.
96. _(correção 19/07/26, Fase 16)_ **Dados pessoais (categorias, contas, cartões) apareciam misturados/"duplicados" para o admin** — reportado pelo usuário ("as categorias têm que ser individuais de cada membro"). **Não havia duplicação no banco**: cada usuário ganha as próprias 14 categorias padrão no cadastro (`create_default_categories`), o que está correto. O problema era no app: **nenhuma query filtrava por `user_id`** — todas dependiam da RLS "own rows", que a decisão 85 afrouxou para `dono OU is_admin_over(user_id)`. Resultado: o admin via as categorias dele **e** as de cada membro na mesma lista (nomes repetidos), além das contas/cartões alheios nos seletores. Regra adotada: **toda query que alimenta seleção, gestão ou export usa `user_id` explícito; só os mapas de EXIBIÇÃO (resolver nome/ícone de uma linha de outro membro) continuam com o escopo ampliado.** Corrigidos: `getCategories` (`/categorias`), seletores de `/orcamentos`, `/recorrentes`, `/cartoes/[id]` (pagar fatura) e `/transacoes/importar`; em `/transacoes`, a página passou a derivar `ownAccounts`/`ownCategories`/`ownCards` (formulário, filtros e select inline) mantendo as listas completas só para os mapas da tabela, e o select inline de categoria vira texto em linha de outro membro (a RLS de escrita recusaria o update). Integridade: `validateRowCategories` (import) e `updateEntryCategory` passaram a exigir que a categoria seja do próprio usuário — antes bastava existir, e o admin agora enxerga as dos membros. **Vazamento corrigido no backup**: `/api/backup` afirmava em comentário que "a RLS já restringe às linhas do usuário" — deixou de ser verdade na Fase 16, e o backup "pessoal" do admin exportaria contas, transações, cartões, orçamentos e metas dos membros junto; todas as 12 queries ganharam `user_id` explícito.
...
| 16  | Família / households (expansão)                                          | ✅ concluída em 19/07/26 (casa, convites por link, RLS estendida, dashboard da família; migration 0014 pende de push; teste com 3 usuários pende do usuário) |
...
## Fase 16 — Família / households (19/07/26)

**Baseada em `docs/ARQUITETURA-EXPANSAO.md`** (decisões 25–30 daquele doc renumeradas para 83–88, conforme a decisão 55). Migration `0014` (pende `db push` + `db:types`). Decisões 83–90.

**Banco (`supabase/migrations/0014_households.sql`):**

- Enums `household_role` (admin/member) e `member_status` (active/removed); tabelas `households`, `household_members` (unique por casa+usuário), `household_invites` (token_hash unique, expiração) e `household_shared_accounts` (unique por casa+conta), com índices do doc.
- 6 funções de parentesco `SECURITY DEFINER` (`is_member_of`, `is_admin_of`, `is_admin_over`, `account_shared_with_me`, `account_owned_by_member_of`, `shares_household_with`) — anti-recursão de policy (decisão 84).
- 3 funções de fluxo `SECURITY DEFINER` (`create_household`, `accept_household_invite`, `get_invite_details`) — `household_members` não tem policy de INSERT de propósito (decisão 87).
- 3 funções de agregado com guarda de membership (`get_household_monthly_summary`, `get_household_category_breakdown`, `get_household_monthly_series`) — mesma semântica de relatório da 0010 (decisão 84).
- RLS: policies completas nas 4 tabelas novas; SELECT estendido em `accounts`/`transactions`/`transaction_installments`/`categories`/`credit_cards`/`credit_card_invoices`/`budgets`/`goals`/`profiles` (decisão 85). Escrita intocada.
- `src/types/database.ts` **hand-patched** (4 tabelas + 2 enums + 13 funções) — substituir por `npm run db:types` após o push.

**`src/features/households/`** (nova feature): `types.ts`, `schemas.ts`, `token.ts` (sha-256 do token de convite), `queries.ts` (`getMyHousehold`, `getInvitePreview`, `getHouseholdDashboard`), `actions.ts` (`createHousehold`, `inviteMember` — gera o link, rate limit 20/h —, `revokeInvite`, `acceptInvite`, `removeMember`, `leaveHousehold` com as guardas da decisão 88, `shareAccount`, `unshareAccount`), `components/` (`create-household-form`, `household-view`, `invite-accept-card`).

**Rotas:** `/familia`, `/familia/dashboard`, `/convite/[token]` (+ middleware protegendo `/familia` e `/convite`). Navegação: "Família" na sidebar; bottom bar mobile reorganizada em 4 itens + "Mais" (decisão 90).

**Verificado:** `npm run lint` ✅ · `npm run typecheck` ✅ · `npm run build` ✅ (28 rotas).

**Critério de pronto (teste manual do usuário, 3 contas reais, após aplicar a `0014`):** admin vê linha a linha dos 3; membro vê agregados + apenas contas compartilhadas; membro NÃO acessa linha não compartilhada; ninguém edita dado alheio; convite expira e reenvio invalida o anterior; membro sai sozinho; único membro encerra a casa.

---

## Histórico de atualizações deste changelog

- **[19/07/26]** — **Correção importante pós-Fase 16**: as telas nunca filtravam por `user_id` (dependiam da RLS "own rows", afrouxada pela decisão 85), então o admin via as categorias/contas/cartões dos membros misturados aos dele — as categorias pareciam "duplicadas" (cada membro tem as próprias 14 padrão; o banco estava correto). Todas as queries de seleção/gestão/export passaram a filtrar pelo próprio usuário, mantendo escopo ampliado só nos mapas de exibição. Inclui vazamento no `/api/backup`, que exportaria os dados dos membros no backup "pessoal" do admin. Decisão 96.
- **[19/07/26]** — Mais ajustes pós-Fase 16 (pedido do usuário): ordenação "Adicionadas por último" (ordem de inserção, via `v_entries.created_at` — migration `0015`) agora é o padrão de `/transacoes`; dashboard principal só mostra movimentações pessoais; dashboard da família ganhou feed de últimas movimentações com o nome do membro (admin); filtro por membro em `/transacoes` para o admin. Decisões 94–95.
- **[19/07/26]** — Melhorias/correções pós-Fase 16 (pedido do usuário): ordenação configurável em `/transacoes` (mais recentes/antigas/maior/menor valor); "Últimas movimentações" do dashboard limitada a `date <= hoje` (parcelas futuras não poluem mais o feed de recentes); abas do drawer de novo lançamento em grid 2 colunas no mobile (não ficam mais cortadas). Decisões 92–93.
- **[19/07/26]** — Correção: `/transacoes` e `/contas` não identificavam de qual membro da família era uma linha/conta vista via RLS estendida (admin ou conta compartilhada) — reportado logo após o teste da Fase 16. `MemberBadge` (nome discreto) aparece só quando a linha não é do próprio usuário; `AccountCard` também esconde ações de escrita em conta alheia. Decisão 91.
- **[19/07/26]** — **🏁 Fase 16 concluída — Família/households (fim do roadmap combinado)**: migration `0014` (4 tabelas, 2 enums, 12 funções DEFINER, RLS de SELECT estendida em 9 tabelas), convites por link com token hasheado (v1 sem e-mail transacional — desvio consciente do doc), rotas `/familia` + `/familia/dashboard` + `/convite/[token]`, bottom bar mobile com menu "Mais" (todas as 11 rotas alcançáveis no celular). Decisões 83–90 (incorporam as 25–30 renumeradas do `ARQUITETURA-EXPANSAO.md`). Lint + typecheck + build verdes (28 rotas). Pende: `db push` + `db:types` e teste de aceite com 3 usuários reais.
- **[19/07/26]** — Correção: vazamento horizontal em várias telas no mobile, achado pelo usuário testando em dispositivo real (prints) — dashboard, `/transacoes` (filtros, seleção em massa, paginação) e diálogos de formulário (color/icon picker) estouravam a largura da tela sem `min-width: 0`. Guard global em `globals.css` + ajustes pontuais de layout. Decisão 82.
- **[19/07/26]** — Correção: compra no cartão (à vista ou parcelada) cuja fatura já estava paga nascia `pending` para sempre — reportado pelo usuário ao lançar uma compra parcelada retroativa. Agora detecta a fatura já paga por item e nasce `paid` + histórica automaticamente. Decisão 81.
